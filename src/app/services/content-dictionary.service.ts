...
export class TextDictionary {
  ...
  private _dictonaryEntries: Entry[] = [
    { title: StarterTitle.DCV, content: `<h3>This section covers single-engine directional control and Vmca.</h3>
    ...
    { title: StarterTitle.ZST, content: `<h3>This section Covers zero sideslip control technique.</h3>`},
    ...
    { title: DCVTitle.Power, content: `<h3>The thrust produced by the operating engine will have a direct effect on the yaw/roll into the inoperative engine. Any decrease in thrust, caused by reducing the power setting and/or an increase in altitude, will decrease the yaw/roll moment, lowering Vmca.</h3>` },
    { title: DCVTitle.DensityAltitude, content: `<h3>Changing the density altitude changes the power output of the engine due to changes in the air density. Increasing altitude (decreasing air density) will result in a loss of power on the operating engine. The yaw/roll moments will be reduced, lowering Vmca.</h3>` },
    { title: DCVTitle.Propeller, content: `<h3>A feathered propeller produces much less drag than a windmilling propeller, decreasing the yaw/roll into the inoperative engine, lowering Vmca.</h3>` },
    ...
    { title: DCVTitle.Airspeed, content: `<h3>Changing airspeed changes the speed of the airflow over the rudder. An increase in airspeed will increase the rudder's effectiveness, making it easier to maintain directional control. The Vmca speed, however, will not be affected.</h3>` },
    ...
    { title: DCVTitle.VMCA, content: `<h3>Vmca is the minimum speed at which directional control can be maintained in flight when one engine suddenly fails and the remaining engine is at takeoff power. The Vmca speed also assumes that not more than 5 degrees of bank is being applied towards the operating engine.</h3>
    ...
    { title: DCVTitle.StallSpeed, content: `<h3>The stall speed will vary with changes in many of the control factors listed. In a light twin like the seminole, it is possible for the stall speed to be higher than Vmca. In this condition, a stall with an inoperative engine may result in a spin and/or an unrecoverable loss of
